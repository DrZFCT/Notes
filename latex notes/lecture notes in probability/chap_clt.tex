...
For $\left \{ \omega:X(\omega)\le x \right \} \subseteq\left \{ \omega:\omega\le F(x) \right \}$, observe if $\omega>F(x)$, then since $F$ is right continuous, $\exists \epsilon>0$ s.t. $F(x+\epsilon)<\omega$. Therefore, $X(\omega)\geq x+\epsilon>x$.
\end{proof}
...
Now we show $\liminf_{n\to\infty} Y_n(x)\geq Y_\infty(x)$. Choose $y<Y_\infty(x)$ s.t. $F_\infty$ is continuous at $y$.  Then  $F_\infty(y)<x$ and $F_n(y)\longrightarrow F_\infty(y)$, so $F_n(y)< x$ for $n$ sufficient large, that is, $Y_n(x)\geq y$. This is true for all such $y$'s so the result follows.\par
...
(ii) For all open sets $G$, $\liminf_{n\to\infty}P(X_n\in G)\geq P(X_\infty\in G)$\par
...
\textbf{vague convergence}
From the test function viewpoint, $C_0(X)$ instead of $C_b(X)$.
\begin{remark}
    Vague convergence has a nice functional analytical interpretation. 
    Let $X$ be a locally compact space.
    By the Riesz representation theorem, the space $M(X)$ of Radon measures 
\end{remark}
...
$\Longrightarrow$: Suppose $F_n$ is not tight. Then there is an $\epsilon>0$ and a subsequence $n(k)\to\infty$ s.t. \[1-F_{n(k)}(k)+F_{n(k)}(-k)\geq\epsilon\] for all $k$. By passing to a further subsequence $F_{n(k_j)}$ we can suppose $F_{n(k_j)}\Longrightarrow_v F$. Let $r<0<s$ be continuity points of $F$. Then $1-F(s)+F(r)=\lim_{j\to\infty} 1-F_{n(k_j)}(s)+F_{n(k_j)}(r)\geq\liminf_{j\to\infty}1-F_{n(k_j)}(k_j)+F_{n(k_j)}(-k_j)\geq\epsilon$. Letting $s\to\infty$ and $r\to-\infty$, we see that $F$ is not the distribution function of a probability measure.
\end{proof}
\begin{corollary}
If there is a $\varphi\geq0$ s.t. $\varphi(x)\to\infty$ as $\left | x \right | \to\infty$ and 
\[\sup_n\int\varphi(x)\mathrm{d}F_n(x)=C<\infty\] then $F_n$ is tight.
\end{corollary}
\begin{proof}
$C\geq\int\varphi(x)\mathrm{d}F_n(x)\geq\inf_{\left | x \right |\geq M}\varphi(x)(F_n(-M)+1-F_n(M))$
\end{proof}
\begin{lemma}
...
\end{theorem}
\begin{lemma}
    If $F_1,\cdots,F_n$ have ch.f. $\varphi_1,\cdots,\varphi_n$ and $\lambda_i\geq 0$ have $\lambda_1+\cdots+\lambda_n=1$, then $\sum_{i=1}^n\lambda_iF_i$ has ch.f. $\sum_{i=1}^n\lambda_i\varphi_i$.
\end{lemma}
...
\begin{example}[Heyde(1963)]
    Consider the lognormal density \[f_0(x)=\frac{1}{\sqrt{(2\pi)}}\frac{1}{x}\exp^{-\frac{(\log x)^2}{2}}1_{x\geq 0}\] 
