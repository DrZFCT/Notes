...
In calculus, two sequence of real numbers, $\left\{a_n\right\}$ and $\left\{b_n\right\}$, satisfy $a_n=O(b_n)$ if and only if $\left|a_n\right|\leq c\left|b_n\right|$ for all $n$ and a constant $c$; and $a_n=o(b_n)$ if and only if $\frac{a_n}{b_n}\to 0$ as $n\to0$.
\begin{definition}
Let $X_1,X_2,\cdots$ be random vectors and $Y_1,Y_2,\cdots$ be random variables defined on a common probability space.\par
(i) $X_n=O(Y_n)$ a.s. if and only if $\PP(\left \| X_n \right \| =O(\left | Y_n \right | ))=1$.\par
(ii) $X_n=o(Y_n)$ a.s. if and only if $\frac{X_n}{Y_n}\to 0$ a.s..\par
(iii) $X_n=O_p(Y_n)$ if and only if for any $\epsilon>0$, there is a constant $C_\epsilon>0$ s.t. $\sup_nP(\left \| X_n \right \| \geq C_\epsilon \left | Y_n \right | )<\epsilon$.\par
...
\begin{lemma}[moments and tails]
    Let $\xi>0$ be an random variable with $\EE\xi\in (0,\infty)$. Then 
    \[(1-r)^2\frac{(\EE\xi)^2}{\EE\xi^2}\leq \PP(\xi>rE\xi)\leq \frac{1}{r},\quad r>0\]
\end{lemma}
\begin{theorem}[convergence in $L^p$ implies convergence in probability]
If $p>0$, then \[\EE\left | Z_n \right | ^p\to 0 \Longrightarrow Z_n\longrightarrow 0\text{ in probability.}\]
\end{theorem}
\begin{proof}
$\PP(\left | Z_n \right |\geq\epsilon)\leq\frac{\EE\left | Z_n \right | ^p}{\epsilon^p}\to 0$
\end{proof}
...
\end{theorem}
\begin{proof}
$\EE(\frac{S_n}{n}-\mu)^2=\var(\frac{S_n}{n})=\frac{1}{n^2}(\sum\var(X_i))\leq \frac{Cn}{n^2}\to 0$
\end{proof}
...
$\sum_{n=1}^\infty \PP(A_n)<\infty\Longrightarrow \PP(A_n\text{ i.o.})=0$.
\end{theorem}
\begin{proof}
...
If the events $A_n$ are independent, then \[\sum_{n=1}^\infty \PP(A_n)=\infty\Longrightarrow \PP(A_n\text{ i.o.})=1\]
\end{theorem}
\begin{proof}
Let $M<N<\infty$. $1-x\leq e^{-x}$ and independence imply $\PP(\bigcap_{n=M}^{N}A_n^c)=\prod_{n=M}^N(1-\PP(A_n))\geq \text{exp}(-\sum_{n=M}^NP(A_n))\to 0$ as $N\to\infty$, so $\PP(\bigcup_{n=M}^{N}A_n)=1,\forall M$. Therefore $\PP(\limsup A_n)=1$.
\end{proof}
\begin{theorem}[Kochen-Stone lemma]
Suppose $\sum_{n=1}^\infty \PP(A_n)=\infty$. If 
\[\limsup_{n\to\infty}\frac{(\sum_{k=1}^nP(A_k))^2}{(\sum_{1\leq i,j\leq n} \PP(A_i\cap A_k))}=\alpha>0\] then $\PP(A_n\text{ i.o.})\geq\alpha$.
\end{theorem}
\begin{remark}
This is a generalization of \ref{The second Borel-Cantelli lemma}.
\end{remark}
\begin{theorem}
If $A_1,A_2,...$ are pairwise independent and $\sum_{n=1}^\infty \PP(A_n)<\infty$, then $\heartsuit$
\end{theorem}
\section{SLLN}
\section{0-1 Laws}
\begin{theorem}[Kolmogorov's 0-1 law]
    If $X_1,X_2,\cdots$ are independent and $A\in\mathcal{T}$, then $\PP(A)=0$ or $1$.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Hewitt-Savage 0-1 law]
    If $X_1,X_2,\cdots$ are i.i.d. and $A\in\mathcal{\EE}$, then $\PP(A)=0$ or $1$.
\end{theorem}
\begin{lemma}
...
\begin{theorem}[Kolmogorov's maximal inequality]
    ...
    \[\PP(\max_{1\leq k\leq n}\left|S_k\right|\geq x)\leq \frac{\text{Var}(S_n)}{x^2}\] 
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    \[\PP(\max_{M\leq m\leq N}\left|S_m-S_M\right|>\epsilon)\leq \epsilon^{-2}\text{Var}(S_N-S_M)=\epsilon^{-2}\sum_{n=M+1}^N\text{Var}(X_n)\] 
    Letting $N\to\infty$, we get \[\PP(\sup_{M\leq m}\left|S_m-S_M\right|>\epsilon)\leq \epsilon^{-2}\sum_{n=M+1}^\infty\text{Var}(X_n)\] 
    If we let $w_M=\sup_{m,n\geq M}\left|S_m-S_n\right|$, then \[\PP(w_M>2\epsilon)\leq \PP(\sup_{M\leq m}\left|S_m-S_M\right|>\epsilon)\to 0\quad\text{as }M\to\infty\]
...
\begin{theorem}[Kolmogorov's three series theorem]
    Let $X_1,X_2,\cdots$ be independent. Let $A>0$ and let $Y_i=X_i1_{\left|X_i\right|\leq A}$. In order that $\sum_{n=1}^\infty X_n$ converges a.s.,
    it is necessary and sufficient that:\newline 
    (i) $\sum_{n=1}^\infty \PP(\left|X_n\right|>A)<\infty$\newline 
...
\begin{proof}
    ...
    Using (ii), $\sum_{n=1}^\infty Y_n$ converges a.s.. (i) and Borel-Cantelli lemma imply $\PP(X_n\ne Y_n\, i.o.)=0$, so $\sum_{n=1}^\infty X_n$ converges a.s..
    \newline 
    For necessity, if the sum of (i) is infinite, $\PP(\left|X_n\right|>A\, i.o.)>0$ and $\lim_{m\to\infty} \sum_{n=1}^m X_n$ can not converge.
...
The next result, show that when $\EE\left|X_1\right|=\infty$, $\frac{S_n}{a_n}$ cannot converge almost surely to a nonzero limit.
\begin{theorem}
    Let $X_1,X_2,\cdots$ be i.i.d. with $\EE\left|X_1\right|=\infty$ and let $S_n=X_1+\cdots+X_n$.
    Let $a_n$ be a sequence of positive numbers with $\frac{a_n}{n}$ increasing
\end{theorem}

\section{Large Deviations}
Let $X_1,X_2,\cdots$ be i.i.d. and let $S_n=X_1+\cdots+X_n$. We will investigate the rate at which $\PP(S_n\geq na)\to 0$ for $x>\mu=EX_i$.
\begin{lemma}
If $\gamma_{m+n}\geq\gamma_m+\gamma_n$, then as $n\to\infty$, $\frac{\gamma_n}{n}\to\sup_m\frac{\gamma_m}{m}$.
\end{lemma}
\begin{theorem}
$\gamma(x)=\lim_{n\to\infty}\frac{\log \PP(S_n\geq nx)}{n}$ exists $\leq 0$.
\end{theorem}
\begin{proof}
Let $\pi_n=\PP(S_n\geq nx)$, then $\pi_{m+n}\geq \PP(S_m\geq mx,S_{n+m}-S_m\geq nx)=\pi_m\pi_n$. Therefore, letting $\gamma_n=\log\pi_n$, from the lemma we conclude the existence of the limit.
\end{proof}
...
\[\phi(t)=\log \EE e^{t\xi},\quad t\in \mathbb{R}\]
