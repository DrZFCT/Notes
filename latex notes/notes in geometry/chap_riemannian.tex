...
\begin{example}[Gradient]
    \[\nabla f=\sum_{i,j}g^{ij}\partial_j f\]  
    \[g(\nabla f,V)=V(f)\]
\end{example}
...
\begin{definition}
    The Riemann curvature is a (1,3) tensor given by
    \begin{equation}
        R(X,Y)Z=\nabla_Y\nabla_X Z-\nabla_X\nabla_Y Z+\nabla_{[X,Y]}Z.
    \end{equation}
\end{definition}
...
\begin{equation}\label{eq:connection-applied-twice}
    \nabla_X(\nabla_Y V)=\nabla^2_{X,Y}V+\nabla_{\nabla_X Y}V
\end{equation}
...
\[ T_{\phi(p)}N=d\phi(T_pM)\oplus [d\phi(T_pM)]^\perp \]
...
\begin{theorem}[Gauss Equation]
    Given $X,Y,Z,W\in\Gamma(M)$, we have 
    \[\bar{\R}(X,Y,Z,W)=\R(X,Y,Z,W)+\left\langle A(X,Z),A(Y,W)\right\rangle -\left\langle A(Y,Z),A(X,W)\right\rangle. \]
\end{theorem}

\begin{theorem}[Codazzi Equation]
    If $U,V,W\in\Gamma(M)$, then 
    \[(\R(U,V)W)^\perp =(\bar{\nabla}_V A)(U,W)-(\bar{\nabla}_U A)(V,W).\]
\end{theorem}

\subsection{Hypersurfaces}
An immersed submanifold $M^m\subset N^n$ is a \textit{hypersurface} when $m=n-1$.
\textit{unit normal} $\bn$ 

Using $\bn$, the second fundamental form        